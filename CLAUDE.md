...
- `navigate_no_spin.xml` — custom behaviour tree with the Spin recovery node removed; it caused
  ...
  **not** re-add `ClearEntireCostmap`: clearing the local costmap erases the obstacle just
  ...
  again in `67a73a5`). The path is replanned only when it becomes invalid (`IsPathValid`), the
  goal changes, or it is older than 20 s — replanning every tick flipped the detour left/right
  around a centred obstacle. 20 s keeps the forced replan off the ~15 s demo path
