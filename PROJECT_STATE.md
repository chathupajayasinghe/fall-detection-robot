...
1.6a FALL-MODEL RETRAINING DATA COLLECTION (sessions 2026-08-17 and 2026-08-30)
...
Dataset now stands at 48 FALL / 83 NOT_FALL (131 windows) in data\index.csv.
Backup refreshed 2026-09-24 to D:\Projects\sketch_apr18a_data_backup_20260817\ — it now mirrors data\ in full, 133 files with data\.claude\ excluded, every file SHA256-verified against the source. The directory NAME still carries the original 2026-08-17 date; its CONTENTS are current.
...
PENDING: ~40 more FALL windows are being collected with the FINAL DEMO RIG — the box attached to the pole, dropped onto a thin mattress. The 48 FALL windows above were BARE-POLE drops, a different mass distribution and a different impact signature from what the demo will actually show, so the existing positives alone would train the model on a rig that is not the one being demonstrated.
