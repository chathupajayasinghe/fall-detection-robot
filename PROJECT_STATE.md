...
Reference scan: re-captured at the scan point with the room clear. 360 beams (1 beam = 1°), ~/maps/reference_scan.npy on the Pi, saved 12:57. Capture pose from a fresh AMCL update (ros2 service call /request_nomotion_update std_srvs/srv/Empty, then /amcl_pose; tf2_echo map base_link agreed): (2.197, 0.006), heading −10.8° (AMCL ESTIMATE — AMCL heading uncertainty at that moment was about ±20°, so this is not a physical measurement). The old home-map reference (26 Jul) is kept on the Pi as ~/maps/reference_scan_home_20260726.npy. Lesson: ros2 topic echo --once /amcl_pose returns the LAST published pose, which can be minutes old if the robot has not moved — check header.stamp, or force a nomotion update first.
⚠️ Door ghost (empty room, before the fix): the door recess showed up as a false "person" (cluster 11–12 beams) because the arrival heading differs from the reference heading and the reference is compared by beam index. One recorded case: AMCL-estimated arrival heading +8.2° at (2.22, −0.11) vs AMCL-estimated reference heading −10.8° gave a false cluster of 11 at base_link (−1.44, −2.94). The ~19° gap between those two numbers is most likely AMCL heading NOISE, not a physical 19° rotation: a real 19-beam shift would push walls, furniture and corners all around the room past DIFF_THRESHOLD and give many large clusters, not a single door-recess ghost — and the ±3° window fix gave 3/3 clean. So the physical heading difference was small, a few degrees.
...
Keep recording the arrival heading of every /find_person run, but treat it as AMCL-ESTIMATED, not physical. The real check is the ROUND-TRIP TEST (still pending): several arrivals at the scan point from the taped origin, each followed by an empty-room /find_person — all must return "No new obstacle".
⚠️ DEMO RULE — the robot must ALWAYS start the mission from the taped origin (0, 0), facing +x. From there it approaches the scan point along the same straight path and arrives with about the same heading as the reference scan. If it started anywhere else it could arrive facing a different way, and the ±3° window would not cover that.
...
⚠️ Robot start position: the robot must start the mission from the taped origin (0, 0), facing +x — the person finder depends on the robot arriving at the scan point with about the same heading as the reference scan (see SESSION 2026-09-25).
...
Correction (2026-09-25): the inline credentials were NOT in D:\Project\falldetect\ or D:\Project\sketch_feb18a\ (both checked 2026-09-24, none found). They were in C:\Users\lap.lk\Desktop\sketch_apr18a\sketch_apr18a.ino (pre-refactor, unversioned): Wi-Fi SSID/password and the Firebase API key. ✅ DELETED 2026-09-25 — the folder was removed and the Recycle Bin emptied. Folders NOT yet checked for credential copies: C:\Users\lap.lk\Desktop\Project_oldfiles\, C:\Users\lap.lk\Desktop\robot\, D:\Project\fall_detector_app\, D:\Projects\StudioProjects\.
